...
const RAISES = [
  { date: new Date('2025-07-01'), rate: 0.035  },
...
function calculateSeries({ plan, dob, enteredSvcMonths, ncSvcMonths = 0, asOfDate, lastDayOfSvc, afcMonthly, slHours, slRate, slAsOf, paystubStream = null }) {
  ...
  // Last month with paystub data — raises on or before this date are already
  ...
  const streamEnd = paystubStream && paystubStream.length
    ? paystubStream[paystubStream.length - 1].month
    : null;
