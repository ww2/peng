...
const RAISES = [
  { date: new Date('2026-07-01'), rate: 0.0379 },
  { date: new Date('2027-07-01'), rate: 0.04   },
  { date: new Date('2028-07-01'), rate: 0.04   },
];
...
function applyRaises(afcMonthly, retDate, N, cutoff = null) {
...
// (the official ERS rule, matching solveDP). Returns null if fewer than N
// such windows can be formed. See PLAN.md Stage 3 for the full algorithm.
function projectAfcAtRetirement({ stream, retDate, currentMonth, raises, N, mode, lastDayOfSvc }) {
  let base = 0;
  ...
  const retMonth = new Date(retDate.getFullYear(), retDate.getMonth(), 1);

  const future = [];
  for (let m = new Date(startMonth); m <= capMonth; m = addMonths(m, 1)) {
    let mult = 1;
    for (const { date, rate } of raises) {
      if (date <= m) mult *= (1 + rate);
    }
    const v = base * mult;
    future.push({ month: new Date(m), regular: v, total: v });
  }

  const past = stream.filter(s => s.month < startMonth && s.month <= retMonth);
...
function calculateSeries({ plan, dob, enteredSvcMonths, ncSvcMonths = 0, asOfDate, lastDayOfSvc, afcMonthly, slHours, slRate, slAsOf }) {
  ...
  for (let m = 0; m < 600; m++) {   // 50-year hard ceiling
    ...
    const raisedAfc = applyRaises(afcMonthly, retDate, config.N, lastDayOfSvc);
    const raisesActive = raisedAfc > afcMonthly;
    ...
    if (slHours != null && slAsOf != null && offElig !== 'ineligible') {
      ...
      if (raisesActive) {
        pensionRaisesCurrentSL   = blendedBenefit(svcAtM + currentMo,   ncSvcMonths, raisedAfc, arf, plan, config);
        pensionRaisesProjectedSL = blendedBenefit(svcAtM + projectedMo, ncSvcMonths, raisedAfc, arf, plan, config);
      }
    }
  ...
  if (lastDayOfSvc && lastDayOfSvc < todayMidnight) {
    const snapMax = key => {
...
function buildPaystubStream(stubs) {
  ...
  const last  = stubs[stubs.length - 1].beginDate;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    PLAN_CONFIGS, PRIMARY_ARF_TABLES,
    // Plan logic
    derivePlanKey, primaryArfAge, primaryEligAge, primaryEligibility, primaryARF,
    isPre1971DualMethod,
    // Date utilities
    toYmd, parseDate, addMonths, addDays, fmtDate, monthsBetween, fractionalAge,
    parseIsoDate, serviceAtMonth, sickLeaveToMonths,
    // Math
    applyRaises, projectAfcAtRetirement, blendedBenefit, calculateSeries,
    // Paystub pipeline
    filterStubs, generateWindows, scoreStub, buildPaystubStream, solveDP, detectGaps,
  };
}
