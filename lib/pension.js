...
function projectAfcAtRetirement({ stream, retDate, raises, N, mode, lastDayOfSvc }) {
  ...
  const futureStart = addMonths(stream[stream.length - 1].month, 1);
...
function calculateSeries({ plan, dob, enteredSvcMonths, ncSvcMonths = 0, asOfDate, lastDayOfSvc, afcMonthly, slHours, slRate, slAsOf, paystubStream = null }) {
  ...
  for (let m = 0; m < 600; m++) {   // 50-year hard ceiling
    ...
    // least one scheduled raise to land inside the projector's cap.
    const raiseCap = lastDayOfSvc
      ? new Date(Math.min(lastDayOfSvc.getTime(), retDate.getTime()))
      : retDate;
    const anyRaiseInHorizon = RAISES.some(r => r.date <= raiseCap);
    const raisesActive = raisedAfc != null && raisedAfc > afcMonthly && anyRaiseInHorizon;
    const hasPaystub = paystubStream && paystubStream.length > 0;
    // When a paystub stream is driving the calculation but raises haven't
    // yet lifted the AFC at this row (early retDate before any raise lands,
    // or NR-dominated past), pin pensionWithRaises to primaryPension so the
    // raise curve overlays the primary from the chart's left edge instead
    // of producing a visible gap. When there's no paystub stream at all,
    // the raise curve is suppressed entirely (null).
    const pensionWithRaises = offElig === 'ineligible' || !hasPaystub ? null
      : raisesActive
