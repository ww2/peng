...
//   payout = min(720, hours-at-retDate) × hourly rate at retDate
// where hours-at-retDate is either the snapshot value (current curve)
// or the snapshot + linear accrual (max-no-spend curve), and the rate
// is `hourlyRateAtAsOf` compounded by every raise strictly after
// `vacAsOfDate` and on/before `retDate`. The 720-hr cap applies at
// all times — both at the as-of snapshot and through the accrual ramp
// — to mirror the year-end carry-over rule.
//
...
// before vacAsOfDate degrade safely: monthsAccrued is clamped at 0 so
// the max curve never dips below the current curve.
//
// `cutoff` (optional): mirrors `applyRaises`. When set, both the raise
// projection and the accrual ramp are capped at min(cutoff, retDate) —
// raises after separation don't apply and vacation accrual stops at
// last day of service.
function vacationPayoutAt(retDate, { vacHoursAsOf, vacAsOfDate, accrualHrsPerMo, hourlyRateAtAsOf, raises, cutoff = null }) {
...
function buildVacationSeries({ vacHours, vacAsOf, vacRate = 14, vacHourlyRate, lastDayOfSvc = null, raisesNA = false }) {
  ...
  if (lastDayOfSvc && lastDayOfSvc < today) {
    ...
    const accruedHours = Math.min(
      VACATION_CAP_HOURS,
      Math.max(0, vacHours + accrualHrsPerMo * Math.max(0, monthsBetween(vacAsOf, lastDayOfSvc)))
    );
    return {
      separated: true,
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    // Math
    applyRaises, projectAfcAtRetirement, vacationPayoutAt, buildVacationSeries, blendedBenefit, calculateSeries,
    // Paystub pipeline
