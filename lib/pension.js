...
//   today's first-of-month → max(LDOS-month, today+48mo)
//   When future LDOS is within 4 years, the horizon still extends to today+4y
//   so the LDOS marker has room to render. When future LDOS is beyond 4 years,
//   the series stops exactly at the LDOS month.
//
// Each row carries two pairs of payouts: the no-raise baseline
// (`currentPayout`, `maxPayout`) and the with-raise variant
// (`currentPayoutWithRaises`, `maxPayoutWithRaises`). Mirrors the pension
// chart's primary / `pensionWithRaises` split so the two pairs render as
// distinct curves and overlay where no raise has lifted the hourly rate yet
// (the with-raise math naturally returns the no-raise value when no raise
// lands in (vacAsOf, retDate]).
//
// The with-raise pair is computed against the real RAISES regardless of the
// `raisesNA` input — this keeps the y-axis ceiling stable when the user
// toggles the "Projected raises do not apply" checkbox (otherwise the
// withRaise pair would collapse to the no-raise pair and the axis would
// shrink). The caller's `raisesNA` preference is echoed in the returned
// `raisesNA` field so the chart can suppress the raise curves while still
// using the lifted values for axis sizing.
//
...
function buildVacationSeries({ vacHours, vacAsOf, vacRate = 14, vacHourlyRate, lastDayOfSvc = null, raisesNA = false }) {
  ...
  const fourYearMark = addMonths(startMonth, 48);
  const ldosMonth    = lastDayOfSvc
    ? new Date(lastDayOfSvc.getFullYear(), lastDayOfSvc.getMonth(), 1)
    : null;
  const endMonth = ldosMonth && ldosMonth > fourYearMark ? ldosMonth : fourYearMark;

  const payoutPair = (retDate, excludeFinalSnap = false) => {
    const opts = {
      vacHoursAsOf: vacHours,
      vacAsOfDate: vacAsOf,
      accrualHrsPerMo,
    ...
    const noRaise   = vacationPayoutAt(retDate, { ...opts, raises: [] });
    const withRaise = vacationPayoutAt(retDate, { ...opts, raises: RAISES });
    return {
      currentPayout:           noRaise.currentPayout,
      maxPayout:               noRaise.maxPayout,
      currentPayoutWithRaises: withRaise.currentPayout,
      maxPayoutWithRaises:     withRaise.maxPayout,
    };
  };

  const rows = [];
  for (let m = startMonth; m <= endMonth; m = addMonths(m, 1)) {
    rows.push({ retDate: m, ...payoutPair(m) });
  }
  ...
  for (let y = startMonth.getFullYear(); y <= endMonth.getFullYear(); y++) {
    ...
    rows.push({ retDate: yearEnd, ...payoutPair(new Date(y + 1, 0, 1), true) });
  }
  rows.sort((a, b) => a.retDate - b.retDate);

  return { separated: false, rows, lastDayOfSvc: lastDayOfSvc ?? null, raisesNA };
}
