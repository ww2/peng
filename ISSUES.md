To simplify my testing, is there any way to add a url param which specifies the path (relative or absolute)
to the paystub directory, so that the page can on reload immediately start scanning them?

Should every raise whose start date is in the past be dropped from the table and projections?

move dob input above the Plan pulldown
...
for each displayed curve (not including COLA), put an X on that curve at the month where
