...
Supported params (URL pre-fill block at `index.html:1906+`): `plan`, `dob`, `memDate`, `svcYears`, `svcMonths`, `ncSvcYears`, `ncSvcMonths`, `svcAsOf`, `lastDay`, `afc`, `slHours`, `slAsOf`, `slRate`, `vacHours`, `vacAsOf`, `vacRate`, `vacHourlyRate`, `cache`. `plan` takes one of three values (`hybrid`, `contributory`, `noncontributory`); the tier qualifier is derived from `memDate` (see "Plan key derivation" below). `ncSvcYears`/`ncSvcMonths` are only meaningful for hybrid plans (NC mixed-service split); they are dropped from the URL on non-hybrid plans. Pre-split URLs containing only `lastDay` are accepted as a shim — that date is also used for `svcAsOf`. The vacation set is all-or-nothing: any of `vacHours` / `vacAsOf` / `vacHourlyRate` filled in requires all three (`vacRate` carries a default of 14 hrs/mo and isn't part of the trigger). `cache` is a presence-only boolean flag (`?cache` with no value) that mirrors the on-screen "Cache paystubs across reloads" checkbox — see "Paystub cache" in "Where Things Live". Bad / unknown params are surfaced in a top-of-page error banner; the corresponding form fields are outlined red until corrected.

## Tests

`node --test tests/*.test.js` from repo root. Two files: `tests/pension.test.js` covers `lib/pension.js` (date utils, paystub pipeline, projector, vacationPayoutAt, calculateSeries wiring). `tests/index-html.test.js` is a smoke test that the inline `<script>` blocks in `index.html` parse cleanly — catches typos without spinning up a browser.
...
2. **Optional adjustments** — sick leave (hours / as-of / accrual rate, default 14 hrs/mo) and vacation (hours / as-of / accrual rate (default 14) / hourly rate). Vacation drives a separate companion chart below the pension chart — see "Vacation chart" below.
3. **Contractual adjustments** — read-only `RAISES` table (`index.html:456`, populated at module-script init from the `RAISES` constant) with a "Projected raises do not apply" override checkbox. The whole fieldset is shown when an effective stream is driving the calculation — either a paystub stream or the synthetic stream that regular-mode plans build from a manual AFC.
4. **Earnings data** — manual monthly AFC OR paystub directory picker (DP solver writes the computed AFC into the manual field)

`canCalculate()` (`index.html:1395`) gates the Generate-graph button on: plan + dob + memDate (range-valid) + svcAsOf + (Still active OR lastDay) + positive AFC + (no SL hours OR slAsOf set) + vacation all-or-nothing across the three user-entered fields (`vacHours`, `vacAsOf`, `vacHourlyRate` — `vacRate` has a default and isn't part of the trigger). `validateMemDate()` (`:1378`) hard-blocks future-dated memDate and soft-flags memDate before DOB or before DOB+18.
...
- **Pension series** — `calculateSeries` at `lib/pension.js:440` accepts `ncSvcMonths` (defaults 0), an optional `paystubStream` (defaults `null`), and an optional vacation set (`vacHours`, `vacAsOf`, `vacRate` (default 14), `vacHourlyRate`). Per-month rows from next month → 10 years past first normal retirement (50-yr ceiling). Each row carries `primaryPension`, `pensionCurrentSL`, `pensionProjectedSL`, `pensionWithRaises`, `pensionRaisesCurrentSL`, `pensionRaisesProjectedSL`, `vacationCurrentPayout`, `vacationMaxPayout`. Already-separated members and future committed `lastDayOfSvc` snap eligible rows to a single value (vacation fields participate in those snap passes too). Internally `calculateSeries` derives an `effectiveStream`: when no `paystubStream` is given but the plan is `mode === 'regular'` and `afcMonthly > 0`, it builds a synthetic flat stream via `buildSyntheticStream` (`lib/pension.js:766`) anchored at today (or `lastDayOfSvc`'s month if already separated) so the raise projector applies to manual-AFC entries on regular-mode plans. Total-mode plans get no synthetic extrapolation — their AFC mixes overtime/NR/differentials that don't scale with contract raises.
...
- **Vacation payout** — `VACATION_CAP_HOURS = 720` constant (`lib/pension.js:55`) and `vacationPayoutAt(retDate, { vacHoursAsOf, vacAsOfDate, accrualHrsPerMo, hourlyRateAtAsOf, raises, cutoff })` (`lib/pension.js:464`). Returns `{ currentPayout, maxPayout, projectedHourlyRate }`. Formula: `min(720, hours-at-effDate) × hourlyRateAtAsOf × Π raises in (vacAsOfDate, effDate]` where `effDate = min(cutoff, retDate)` if cutoff set else retDate. The 720-hr cap applies everywhere (carry-over rule between calendar years), so both the as-of snapshot and the running accrual ramp are clamped to it. Past-raise filter is the as-of date (raises ≤ vacAsOf are already baked into the entered hourly rate). `cutoff` mirrors `applyRaises`: when `lastDayOfSvc` is set, it caps both raise compounding and accrual at separation. `calculateSeries` populates the two payout fields on **every row** (not just eligible) so the pre-cap accrual ramp is visible through compressed zones in the companion chart.
...
- **Paystub pipeline** — `filterStubs` (`lib/pension.js:637`), `generateWindows` (`:667`), `scoreStub` (`:697`), `buildPaystubStream` (`:723`, truncates trailing months not anchored by a stub whose `endDate` is the last day of its month — returns `[]` if no anchor exists), `buildSyntheticStream` (`:766`, the manual-AFC analogue used by regular-mode extrapolation — see "Pension series"), `solveDP` (`:776`), `detectGaps` (`:815`). The non-pure end of the pipeline (`computeAndFillAfc`) lives at `index.html:1133`.
...
- **Contractual-adjustments fieldset** — `#group-contractual-input` (`index.html:450`) is hidden by default (HTML `hidden` attribute) and toggled at runtime by `setContractualVisible` (`index.html:1741`); shown when `raisesProjected` is true, where `raisesProjected = paystubStream !== null || extrapolating` and `extrapolating = !paystubStream && planConfig?.mode === 'regular' && manualAfc > 0` (`index.html:1509`). `runCalculate` derives `showRaises = raisesProjected && !raisesNA` and threads it to `drawChart` and `drawSeriesTable`. The "Projected raises do not apply" auto-lock (`applyRaisesNALock`, `:1720`) fires when `lastDayOfSvc` cuts off all raises in the projection horizon — for paystub OR synthetic-extrapolation streams.
- **Chart** — `drawChart` at `index.html:2030`. Curves drawn back-to-front via `makeCurve`.
- **Vacation chart** — `drawVacationChart` (separate `<svg id="vac-chart-svg">` below the pension chart). Reads `vacationCurrentPayout` / `vacationMaxPayout` directly off the series rows; visibility toggles via the `.has-data` class on the SVG (hidden when no row has a vacation value). Same x-axis treatment as the pension chart (compression, region shading, year labels) so the two stack with aligned x-axes. Own y-axis in `$` lump sum, own legend below the x-axis label, own hover tooltip (no COLA — one-time payout).
- **Shared x-scale helper** — `buildChartXScale(series, iW)` near drawChart returns `{ COMPRESS_PX, xMin, xMax, compressedSegs, eligPxStart, eligPxWidth, eligFromDate, xScale }` (with `xScale.invert` for tooltip use). Both `drawChart` and `drawVacationChart` call it so their x-axes stay byte-for-byte identical.
...
- **Vacation chart tooltip**: separate from the pension chart's. Two crosshair dots (current + max) plus a centered top label of the form `Aug 2030 — $32,400 (max), $20,000 (current)` colored per curve. No COLA — vacation is a one-time payout.
- **Estimation table** (`drawSeriesTable` at `:1444`) — first 48 rows; collapses tail rows once values stabilize. Vacation columns ("vac. (current)", "vac. (max)") appended when `hasVac` is true; they participate in the stabilization fingerprint.
